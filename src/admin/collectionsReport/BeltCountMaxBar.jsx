...
const BeltCountMaxBar = () => {
    const {data} = useContext(ReportsContext)
    const {maxAwardCounts} = data.collectionsStatsCurrent.allUsers

    const barData = awards.map(award => {
        if( award.rank > 19) return null
        return {
            id: award.name,
            label: award.makeModels[0].model,
            count: maxAwardCounts[award.id] || 0,
            value: maxAwardCounts[award.id] || 0
        }
    }).filter(x => x)
